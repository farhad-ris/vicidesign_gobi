...
public abstract class JSONClassEdit extends AsyncTask<String, Void, String>{
	...
	public static final String TAG = JSONClassEdit.class.getSimpleName();
	...
	protected void onPostExecute(String result) {
		...
		try {
			...
			new GobiToast(context, "SYNC TASK EDIT");
			...
			for (i = 0; i < names.length(); i++) {
				if (!names.getString(i).equals("Result")) {
					...
					if(hasAssignee){
						...
						newAssignee = task.getString("NEW");
			...
			//update Task table
			//------------------------------------TASK ASSIGNEE-------------------------------------------------
			boolean insertAssignee = Boolean.valueOf(newAssignee);
			if(hasAssignee){
				if(insertAssignee){
					resolver.insert(GobiContentProvider.CONTENT_URI_TASKASSIGNEE, valuesForAssign);
				}else{
					resolver.update(GobiContentProvider.CONTENT_URI_TASKASSIGNEE, valuesForAssign, DatabaseHandler.KEY_TASKID+" = "+ values.getAsInteger(DatabaseHandler.KEY_TASKID), null);
				}
			...
			resolver.update(GobiContentProvider.CONTENT_URI_TASK, values, DatabaseHandler.KEY_TASKID+" = "+ values.getAsInteger(DatabaseHandler.KEY_TASKID), null);
